...
function ParamsScreen({ state, setState, sessionId, onNext, onBack }) {
  ...
  const handleProcess = async () => {
    ...
    try {
      ...
      const res = await fetch(`${base}/api/session/${sessionId}/process`, {
        method: 'POST',
    ...
    <div className="params-shell">
      <main className="params-main">
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 32, paddingTop: 18, borderTop: '1px solid var(--line)' }}>
          ...
          {startError && (
            <div style={{ color: '#ff6464', fontFamily: 'var(--mono)', fontSize: 11, alignSelf: 'center' }}>
              ⚠ {startError}
            </div>
          )}
          <button className="btn" onClick={handleProcess} disabled={starting}>
            {starting ? 'Iniciando…' : 'Mezclar & masterizar'}<span className="arrow">→</span>
          </button>
        </div>
      </main>
